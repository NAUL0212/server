...
export default {
    ...
    computed: {
        ...
        // Lọc sản phẩm thuộc danh mục "bottoms"
        paginatedProducts() {
            const bottoms = this.productsByCategory('bottoms');  // Lấy sản phẩm thuộc "bottoms"
            const start = (this.$store.state.currentPage - 1) * this.$store.state.itemsPerPage;
            const end = start + this.$store.state.itemsPerPage;
            return bottoms.slice(start, end);  // Phân trang sản phẩm "bottoms"
        }
    },
